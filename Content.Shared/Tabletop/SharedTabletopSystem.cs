...
public abstract partial class SharedTabletopSystem : EntitySystem
{
    ...
    private void OnInteractUsing(Entity<TabletopGameComponent> ent, ref InteractUsingEvent args)
    {
        ...
        if (!TryComp<HandsComponent>(args.User, out var hands)) // Trauma - use trycomp
            return;

        if (ent.Comp.Session is not { } session)
            return;

        if (!_hands.TryGetActiveItem((args.User, hands), out var handEnt)) // Trauma - pass the user instead, chess board has no hands
            return;
